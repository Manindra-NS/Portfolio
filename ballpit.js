// ballpit.js
// Vanilla JS port of the ReactBits <Ballpit /> component.
// Physics-based ball pit rendered with Three.js instanced meshes.
//
// Requires an import map in the HTML that resolves "three" and
// "three/addons/" — see projects.html.
...
// Timer lived in examples/addons (not core) until three.js folded it
// into core in a later revision than the v0.160 pinned in the import
// map, so it has to be imported from its addon path here.
import { Timer as e } from 'three/addons/misc/Timer.js';
...
class Z extends d {
  constructor(e, t = {}) {
    ...
    // envMapRotation was added to materials in a later three.js
    // revision than the v0.160 pinned in the import map — guard it
    // so this still works on that version (env map just keeps its
    // default orientation instead of being rotated).
    if (r.envMapRotation) r.envMapRotation.x = -Math.PI / 2;
    super(o, r, i.count);
    this.config = i;
...
/**
 * Mounts the Ballpit animation inside a container element.
 * The container should have a defined width/height (e.g. a fixed
 * fullscreen background div) since the canvas sizes itself to match
 * the container's bounding box.
 *
 * @param {HTMLElement} container
 * @param {Object} [options]
 * @param {number} [options.count=200] - number of spheres
 * @param {number} [options.gravity=0.5]
 * @param {number} [options.friction=0.9975]
 * @param {number} [options.wallBounce=0.95]
 * @param {boolean} [options.followCursor=true] - if false, the cursor-controlled sphere is invisible (but still influences physics)
 * @param {number[]} [options.colors=[0,0,0]] - hex numbers, e.g. [0xA855F7, 0xB497CF]
 * @returns {Function} cleanup function - stops rendering and removes the canvas
 */
export function initBallpit(container, options = {}) {
  ...
  const instance = createBallpit(canvas, options);
