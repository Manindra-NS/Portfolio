// prism.js
// Vanilla JS port of the ReactBits <Prism /> component.
// Loads `ogl` from a CDN as an ES module, same approach as aurora.js / ferrofluid.js.
...
/**
 * Mounts the Prism WebGL animation inside a container element.
 * @param {HTMLElement} containerEl
 * @param {Object} [options]
 * @param {number} [options.height=3.5]
 * @param {number} [options.baseWidth=5.5]
 * @param {'rotate'|'hover'|'3drotate'} [options.animationType='rotate']
 * @param {number} [options.glow=1]
 * @param {{x:number,y:number}} [options.offset={x:0,y:0}]
 * @param {number} [options.noise=0.5]
 * @param {boolean} [options.transparent=true]
 * @param {number} [options.scale=3.6]
 * @param {number} [options.hueShift=0]
 * @param {number} [options.colorFrequency=1]
 * @param {number} [options.hoverStrength=2]
 * @param {number} [options.inertia=0.05]
 * @param {number} [options.bloom=1]
 * @param {boolean} [options.suspendWhenOffscreen=false]
 * @param {number} [options.timeScale=0.5]
 * @returns {Function} cleanup function
 */
export function initPrism(containerEl, options = {}) {
  const {
    height = 3.5,
    baseWidth = 5.5,
  ...
  containerEl.classList.add('prism-container');
  ...
  // Cleanup: stops the animation, removes listeners/observers, removes the canvas
  return () => {
    stopRAF();
    ro.disconnect();
    if (animationType === 'hover') {
    ...
    if (gl.canvas.parentElement === containerEl) containerEl.removeChild(gl.canvas);
